require('source-map-support').install(); //For ASCII banners:
...
lens.Vector3 = function () {
    ...
    v.prototype.normal = function () {
        var len = this.length();
        return new lens.Vector3(this.x / len, this.y / len, this.z / len);
    };
    v.prototype.add = function (v$2) {
        return new lens.Vector3(this.x + v$2.x, this.y + v$2.y, this.z + v$2.z);
    };
    v.prototype.sub = function (v$2) {
        return new lens.Vector3(this.x - v$2.x, this.y - v$2.y, this.z - v$2.z);
    };
    v.prototype.cross = function (v$2) {
        var tx = this.y * v$2.z - this.z * v$2.y;
        var ty = this.z * v$2.x - this.x * v$2.z;
        var tz = this.x * v$2.y - this.y * v$2.x;
        return new lens.Vector3(tx, ty, tz);
    };
    v.prototype.dot = function (v$2) {
        return this.x * v$2.x + this.y * v$2.y + this.z * v$2.z;
    };
    v.prototype.mul = function (s) {
        return new lens.Vector3(this.x * s, this.y * s, this.z * s);
    };
...
lens.Color = function () {
    ...
    c.prototype.mul = function (s) {
        var r = new lens.Color(this.r * s, this.g * s, this.b * s, this.a);
        return r;
    };
    c.prototype.add = function (c$2) {
        var nc = new lens.Color(this.r + c$2.r, this.g + c$2.g, this.b + c$2.b, this.a);
        return nc;
    };
    return c;
}();
...
lens.Triangle = function () {
    var tr = function (v0, v1, v2, color) {
        ...
        this.color = color || new lens.Color();
        this.n = v1.sub(v0).cross(v2.sub(v0)).normal();
    };
...
lens.Section = function () {
    var sct = function (left, top, width$2, height$2) {
        this.left = left || 0;
        this.top = top || 0;
...
lens.Resolution = function () {
    var rs = function (width$2, height$2) {
        this.width = width$2 || 640;
        this.height = height$2 || 480;
    };
    return rs;
}();
lens.Job = function () {
...
lens.Renderer = function () {
    ...
    rndr.prototype.getRay = function (x, y) {
        ...
        var screen_point = this.job.scene.camera.left_top.add(this.hdv.mul(x)).add(this.vdv.mul(y));
        var direction = screen_point.sub(this.job.scene.camera.eye).normal();
    ...
    rndr.prototype.refractedRay = function (ray, result) {
        ...
        if (ta <= 1) {
            result.ref_idx = 1.5;
            var direction = ray.direction.mul(n).sub(result.normal.mul(n + Math.sqrt(1 - ta)));
    ...
    rndr.prototype.trace = function (ray) {
        ...
        this.job.scene.objects.forEach(function (obj) {
            ...
            if (trc.hit === true && trc.distance < result.distance) {
                ...
                result.hit_point = ray.origin.add(ray.direction.mul(result.distance));
    ...
    rndr.prototype.shade = function (ray, result) {
        ...
        this.job.scene.lights.forEach(function (li) {
            ...
            if (that.traceFirstHit(shadow_ray, max_distance) === false) {
                ...
                if (tc > 0) {
                    result.color = result.color.add(result.obj.color.mul(tc));
                }
            }
        });
    };
    rndr.prototype.render = function (job, buffer) {
        var j = this.job = job;
        var s = j.section;
        this.hdv = j.scene.camera.right_top.sub(j.scene.camera.left_top).div(j.resolution.width);
        this.vdv = j.scene.camera.left_bottom.sub(j.scene.camera.left_top).div(j.resolution.height);
        for (var y = 0; y < s.height; ++y) {
            for (var x = 0; x < s.width; ++x) {
                ...
                if (result.hit) {
                    ...
                    buffer[y * s.width + x] = new lens.Color(0.2, 0.2, 0.2, 1);
            }
        }
    };
    return rndr;
}();
function cornellBox(width$2, height$2, length, center, scene) {
    var w = width$2;
    var h = height$2;
    var l = length;
    var x = center.x;
    var y = center.y;
    var z = center.z;
    var back1 = new lens.Triangle(new lens.Vector3(-w + x, -h + y, l + z), new lens.Vector3(-w + x, h + y, l + z), new lens.Vector3(w + x, -h + y, l + z), new lens.Color(1, 1, 1, 1));
    var back2 = new lens.Triangle(new lens.Vector3(-w + x, h + y, l + z), new lens.Vector3(w + x, h + y, l + z), new lens.Vector3(w + x, -h + y, l + z), new lens.Color(1, 1, 1, 1));
    var bottom1 = new lens.Triangle(new lens.Vector3(-w + x, -h + y, l + z), new lens.Vector3(w + x, -h + y, l + z), new lens.Vector3(w + x, -h + y, -l + z), new lens.Color(1, 1, 1, 1));
    var bottom2 = new lens.Triangle(new lens.Vector3(-w + x, -h + y, l + z), new lens.Vector3(w + x, -h + y, -l + z), new lens.Vector3(-w + x, -h + y, -l + z), new lens.Color(1, 1, 1, 1));
    var top1 = new lens.Triangle(new lens.Vector3(-w + x, h + y, l + z), new lens.Vector3(w + x, h + y, -l + z), new lens.Vector3(w + x, h + y, l + z), new lens.Color(1, 1, 1, 1));
    var top2 = new lens.Triangle(new lens.Vector3(-w + x, h + y, l + z), new lens.Vector3(-w + x, h + y, -l + z), new lens.Vector3(w + x, h + y, -l + z), new lens.Color(1, 1, 1, 1));
    var left1 = new lens.Triangle(new lens.Vector3(-w + x, -h + y, -l + z), new lens.Vector3(-w + x, h + y, -l + z), new lens.Vector3(-w + x, -h + y, l + z), new lens.Color(1, 0, 0, 1));
    var left2 = new lens.Triangle(new lens.Vector3(-w + x, h + y, -l + z), new lens.Vector3(-w + x, h + y, l + z), new lens.Vector3(-w + x, -h + y, l + z), new lens.Color(1, 0, 0, 1));
    var right1 = new lens.Triangle(new lens.Vector3(w + x, -h + y, -l + z), new lens.Vector3(w + x, -h + y, l + z), new lens.Vector3(w + x, h + y, -l + z), new lens.Color(0, 1, 0, 1));
    var right2 = new lens.Triangle(new lens.Vector3(w + x, h + y, -l + z), new lens.Vector3(w + x, -h + y, l + z), new lens.Vector3(w + x, h + y, l + z), new lens.Color(0, 1, 0, 1));
    scene.addObj(back1);
    scene.addObj(back2);
...
function spheresPyramid(x_size, z_size, y_size, radius, color, base_center, scn) {
    var x = 0;
    var y = 0;
    var z = 0;
    var rad = radius * 1.7;
    for (var k = 0; k < y_size; ++k) {
        ...
        for (var j = 0; j < z_size; ++j) {
            ...
            for (var i = 0; i < x_size; ++i) {
                ...
                var sp = new lens.Sphere(new lens.Vector3(x, y, z), radius, color);
                scn.addObj(sp);
            }
        }
        x_size -= 1;
...
lens.SceneDemo1 = function () {
    ...
    spheresPyramid(3, 3, 3, 1, new lens.Color(1, 1, 0, 1), new lens.Vector3(0, -3, 0), scn);
    //Blue sphere
    /*
...
*/
    scn.addLight(new lens.PointLight(new lens.Vector3(5, 0, -10)));
    //4:3 camera
    var cam = new lens.Camera(new lens.Vector3(-6.4, -5.2, -12), new lens.Vector3(-6.4, 4.4, -12), new lens.Vector3(6.4, 4.4, -12), new lens.Vector3(0, -2, -22));
    scn.camera = cam;
    return scn;
};
